...
test("remembers an explicit user collapse so a card can stay closed", () => {
  clearExpandedToolCalls();
  assert.equal(isToolCallCollapsedByUser("call_1"), false);

  setToolCallExpanded("call_1", false);
  assert.equal(isToolCallCollapsedByUser("call_1"), true);
  assert.equal(isToolCallExpanded("call_1"), false);

  // Expanding again clears the collapse, so the two sets never disagree.
  setToolCallExpanded("call_1", true);
  assert.equal(isToolCallCollapsedByUser("call_1"), false);
  assert.equal(isToolCallExpanded("call_1"), true);

  setToolCallExpanded(undefined, false);
  assert.equal(isToolCallCollapsedByUser(undefined), false);
});
