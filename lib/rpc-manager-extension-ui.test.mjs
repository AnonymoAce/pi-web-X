...
test("multi-select resolves to the chosen values and cancels to undefined", async (t) => {
  const { wrapper, ui, events } = setup(t);

  const pending = ui.multiSelect("Pick some", ["Alpha", "Beta", "Gamma"]);
  const request = events.at(-1);
  assert.equal(request.method, "multi-select");
  assert.deepEqual(request.options, ["Alpha", "Beta", "Gamma"]);
  await wrapper.send({ type: "extension_ui_response", id: request.id, values: ["Alpha", "Gamma"] });
  assert.deepEqual(await pending, ["Alpha", "Gamma"]);
  assert.deepEqual(events.at(-1), { type: "extension_ui_closed", id: request.id });

  const cancelled = ui.multiSelect("Pick some", ["Alpha"]);
  const cancelledRequest = events.at(-1);
  await wrapper.send({ type: "extension_ui_response", id: cancelledRequest.id, cancelled: true });
  assert.equal(await cancelled, undefined);

  // A caller abort keeps the undefined default and leaves no pending request behind.
  const controller = new AbortController();
  const aborted = ui.multiSelect("Pick some", ["Alpha"], { signal: controller.signal });
  controller.abort();
...
test("multi-select honours the dialog timeout with the undefined default", async (t) => {
  const { wrapper, ui, events } = setup(t);
  const pending = ui.multiSelect("Pick some", ["Alpha"], { timeout: 10 });
  assert.equal(events.at(-1).method, "multi-select");
  assert.ok(events.at(-1).expiresAt > Date.now());
